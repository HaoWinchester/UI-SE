...
export class DeliveryOrchestrator {
  ...
  async createJob(rawRequirement: string): Promise<WorkflowJob> {
    ...
    const specArtifact = await this.persistSpecArtifact(
      ...
      specExecution.result.data.pendingQuestions.length,
    );
  ...
  async run(jobId: string): Promise<WorkflowJob> {
    ...
    if (
      !initialJob.specArtifact?.checklistPassed ||
      (initialJob.specArtifact?.pendingClarificationCount ?? 0) > 0
    ) {
      return this.blockJob(
        jobId,
        `Spec quality gate failed. Pending clarifications: ${
          initialJob.specArtifact?.pendingClarificationCount ?? 0
        }. Checklist passed: ${initialJob.specArtifact?.checklistPassed ?? false}.`,
      );
    }
    ...
    if (approvedUiArtifact.runtime !== "real") {
      return this.blockJob(
        jobId,
        `Approved UI was not generated by real Stitch. Current runtime: ${approvedUiArtifact.runtime}. ${
          approvedUiArtifact.note ?? ""
        }`.trim(),
      );
    }
    ...
    await this.markSpeckitFinalTask(jobId, "flowValidation");
    ...
    await this.markSpeckitFinalTask(jobId, "acceptance");
  ...
  private async persistSpecArtifact(
    ...
    return {
      ...
      clarifiedAt,
      checklistPassed: speckitWorkspace.checklistPassed,
      checklistNotes: speckitWorkspace.checklistNotes,
      pendingClarificationCount: speckitWorkspace.pendingClarificationCount,
      ...
      speckitPlanPath: undefined,
      speckitResearchPath: undefined,
      speckitDataModelPath: undefined,
      speckitContractsDirPath: undefined,
      speckitQuickstartPath: undefined,
      speckitTasksPath: undefined,
    };
  }

  private async prepareSpeckitImplementationPhase(jobId: string, approvedUiArtifact: UiArtifact): Promise<void> {
    const job = await this.deps.store.get(jobId);
    if (!job.specArtifact) {
      throw new Error("Spec artifact is missing; cannot prepare Speckit implementation artifacts.");
    }

    const planningWorkspace = await prepareSpeckitImplementationWorkspace(
      this.deps.baseDir,
      job.requirement,
      job.specArtifact,
      job.codeWorkspace,
    ...
    await this.deps.store.update(jobId, (current) => ({
      ...current,
      specArtifact: current.specArtifact
        ? {
            ...current.specArtifact,
            speckitPlanPath: planningWorkspace.planPath,
            speckitResearchPath: planningWorkspace.researchPath,
            speckitDataModelPath: planningWorkspace.dataModelPath,
            speckitContractsDirPath: planningWorkspace.contractsDirPath,
            speckitQuickstartPath: planningWorkspace.quickstartPath,
            speckitTasksPath: planningWorkspace.tasksPath,
          }
        : current.specArtifact,
    }));
    ...
    await this.persistWorkflowLog(jobId, {
      ...
      message: "Generated Speckit plan/tasks artifacts for implementation.",
      teamName: "requirement-design-team",
      teamLabel: agentTeams["requirement-design-team"].label,
      details: {
        planPath: planningWorkspace.planPath,
        tasksPath: planningWorkspace.tasksPath,
      },
    });
    await this.refreshDashboard(jobId);
  }
  ...
  private async markSpeckitFinalTask(
    jobId: string,
    step: "flowValidation" | "acceptance",
  ): Promise<void> {
    const job = await this.deps.store.get(jobId);
    const tasksPath = job.specArtifact?.speckitTasksPath;
    if (!tasksPath) {
